                ...
                <el-card>
                    ...
                    <ul class="news-list" v-loading="newsLoading">
                        <li v-for="news in latestNews" :key="news.url" class="news-item" @click="openNews(news.url)">
                            ...
                            <span class="news-time">{{ formatTime(news.publishTime || news.time) }}</span>
                        </li>
                    </ul>
                </el-card>
            </el-col>
...
const indices = ref([
    { name: '上证指数', value: '--', change: 0, percent: 0, symbol: 'sh000001', stockName: '上证指数' },
    { name: '深证成指', value: '--', change: 0, percent: 0, symbol: 'sz399001', stockName: '深证成指' },
    { name: '创业板指', value: '--', change: 0, percent: 0, symbol: 'sz399006', stockName: '创业板指' }
])
...
const fetchIndexLatestData = async () => {
    for (let i = 0; i < indices.value.length; i++) {
        const idx = indices.value[i]
        try {
            const res = await getStockHistory(idx.symbol, 'D')
            if (Array.isArray(res.data) && res.data.length > 0) {
                const latestData = res.data[res.data.length - 1]
                const prevData = res.data.length > 1 ? res.data[res.data.length - 2] : latestData
                ...
                indices.value[i] = {
                    ...idx,
                    value: closePrice.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }),
                    change: parseFloat(change.toFixed(2)),
                    percent: parseFloat(percent.toFixed(2))
                }
            }
        } catch (error) {
            console.error(`Failed to fetch data for ${idx.symbol}:`, error)
        }
    }
}
